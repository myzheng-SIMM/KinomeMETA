...
#         self.mol_attend = nn.ModuleList([nn.Linear(fingerprint_dim, fingerprint_dim) for t in range(T)])
    ...
    def forward(self, atom_list, bond_list, atom_degree_list, bond_degree_list, atom_mask, y_target=None):
...
#             print(mol_feature.shape,mol_feature)
        ...
        if y_target is not None:
            ...
            y_target_adjust = np.array([y_target[v] for v in validInds]).astype(float)
            validInds = torch.cuda.LongTensor(validInds).squeeze()

            pred_ = torch.index_select(mol_prediction, 0, validInds)  
            loss = self.loss_function(pred_, torch.cuda.LongTensor(y_target_adjust))

            return loss, pred_, y_target_adjust
        else:
